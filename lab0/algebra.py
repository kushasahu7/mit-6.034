...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    '*' will not help you.
    """
    print("exp 1", expr1, "ep 2", expr2)
    if isinstance(expr1, Sum) is True and isinstance(expr2, Sum) is True:
        terms = []
        for x in expr1:
            for y in expr2:
                terms.append(Product[x,y])
        return Product(terms)
    elif isinstance(expr1, Product) is True and isinstance(expr2, Product) is True:
        terms = []
        for x in expr1:
            terms.append(x)
        for y in expr2:
        ...
        sumExp = expr1 if isinstance(expr1, Sum ) else expr2
        prodExp = expr1 if isinstance(expr1, Product) else expr2
        terms = []
        for x in sumExp:
            print("x", x, prodExp)
            terms.append(Product([x, prodExp]).flatten())
        return Sum(terms)
