...
# discussed in class.
from production import DELETE, IF, AND, OR, NOT, THEN, forward_chain
...
fatherRule = IF( AND( 'parent (?x) (?y)',
                 'male (?x)' ),
            THEN( 'father (?x) (?y)' ) )
motherRule = IF( AND( 'parent (?x) (?y)',
                 'female (?x)' ),
            THEN( 'mother (?x) (?y)' ) )

siblingRule = IF( AND( 'parent (?x) (?y)',
                  'parent (?x) (?z)', NOT('same (?y) (?z)')),
            THEN( 'sibling (?y) (?z)' ) )
sonRule = IF( AND( 'parent (?x) (?y)',
                 'male (?y)' ),
            THEN( 'son (?y) (?x)' ) )


dauRule = IF( AND( 'parent (?x) (?y)',
                 'female (?y)' ),
            THEN( 'daughter (?y) (?x)' ))

sisterRule = IF( AND( 'sibling (?y) (?x)',
                      'female (?y)' ),
            THEN( 'sister (?y) (?x)' ) )        

broRule = IF( AND( 'sibling (?y) (?x)',
                      'male (?y)' ),
            THEN( 'brother (?y) (?x)' ) )

cousinbRule = IF( AND( 'parent (?a) (?b)',
                      'parent (?c) (?d)',
                 'sibling (?a) (?c)'),
            THEN( 'cousin (?b) (?d)' ) )
family_rules = [ 
...
#print("TEST_RESULTS_6", TEST_RESULTS_6)
...
#print("black_family_cousins", black_family_cousins)
...
#print("TEST_RESULTS_2", TEST_RESULTS_2)
...
WHAT_I_FOUND_INTERESTING = 'Programming wise: how the IF, THEN, DELET clases were implemented'
